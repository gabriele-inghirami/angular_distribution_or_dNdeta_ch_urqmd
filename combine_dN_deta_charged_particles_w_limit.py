...
with open(infile,"r") as ff:
    ...
    discarded_events = int(ff.readline().split()[3])
...
dN = np.array(dN_list)
total_included_events = events
total_discarded_events = discarded_events
...
fileout.write("# events: "+str(total_included_events)+"\n")
fileout.write("# discarded events: "+str(total_discarded_events)+"\n")
fileout.write("# deta: "+str(deta)+"\n")
fileout.write("# eta    dN/deta\n")
for i in range(eta.size):
    fileout.write("{:5.2f}".format(eta[i])+"  "+"{:8.2f}".format(dN[i]/(total_included_events*deta))+"\n")
fileout.close()
